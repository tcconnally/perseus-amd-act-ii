...
# --- inference on AMD Instinct (Fireworks AI), budget-guarded ----------------
# The memory layer above is CPU-only. The agent's ANSWER is generated by an
# open-weight LLM served on AMD Instinct via the Fireworks AI API. Because this is a
# public endpoint spending a shared credit balance, calls are hard-capped per day and
# in total; when the cap is hit (or no key is set) we fall back to a labelled
# memory-grounded composition and never fabricate an LLM answer.
...
def infer(query: str, grounding: str):
    """Return (answer, backend_label, used_llm). Real LLM on AMD when keyed and under
    budget; otherwise an honest, clearly-labelled memory-grounded composition."""
    if FIREWORKS_API_KEY and grounding and _budget_ok():
        ...
        try:
            ...
            if text:
                ...
                return text, f"{short} on AMD Instinct via Fireworks AI (live)", True
        except Exception:
...
<style>
  ...
  *{box-sizing:border-box} body{margin:0;background:var(--bg);color:var(--ink);
  ...
  <p class="sub">Encrypted, local-first agent memory — kept <b>off the GPU</b>. Recall runs on the
     <b>host CPU</b>; the agent's answer is generated live on <b>AMD Instinct via Fireworks AI</b>.<br>
     <b>Judges:</b> tap an example below to teach a fact, then hit <b>Recall</b> — and read the
     AMD-generated answer.</p>

  <div class="banner">⚠︎ Honesty: recall &amp; footprint are <b>measured live on this CPU</b>
     (<span class="gpu0">0 bytes of GPU HBM</span>). The agent's <b>answer</b> is generated by an
     open-weight LLM served on <b>AMD Instinct via Fireworks AI</b> (with a daily budget cap; it
     falls back to a labelled memory-grounded composition when reached). The MI300X cost table is a
